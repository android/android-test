...
def _development_repositories():
    ...
    if not native.existing_rule("robolectric"):
        http_archive(
            name = "robolectric",
    ...
    if not native.existing_rule("jsr330"):
        http_archive(
            name = "jsr330",
            build_file_content = """
...
def android_test_repositories(with_dev_repositories = False):
    ...
    if not native.existing_rule("google_apputils"):
        http_archive(
    ...
    if not native.existing_rule("gflags_archive"):
        http_archive(
            name = "gflags_archive",
    ...
    if not native.existing_rule("portpicker_archive"):
        http_archive(
    ...
    if not native.existing_rule("mox_archive"):
        http_archive(
            name = "mox_archive",
    ...
    if not native.existing_rule("six_archive"):
        x provides simple utilities for wrapping over differences between Python 2 and Python 3.
        http_archive(
            name = "six_archive",
