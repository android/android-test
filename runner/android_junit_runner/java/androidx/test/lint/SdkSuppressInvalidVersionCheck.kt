 ...
 * [android.os.Build.VERSION_CODES.CUR_DEVELOPMENT]), which represent the under-development SDK.
 *
 * For example,
 * ```java
 ...
 * ```
 *
 * Comparing the SDK version with the under-development version code is meaningless. Should replace
 * the invalid version codes to versions of finalized SDKs.
 */
  ...
  private fun checkInvalidSdkVersion(node: UElement) =
    (node is ULiteralExpression && node.value is Int && node.value in INVALID_VERSION_INT_LIST) ||
      (node is UQualifiedReferenceExpression &&
        node.selector is USimpleNameReferenceExpression &&
        (node.selector as USimpleNameReferenceExpression).identifier in INVALID_VERSION_STRING_LIST)

  companion object {
    private val REPORT_MESSAGE =
      """
        You have an invalid version code VERSION_PLACEHOLDER for attribute ATTRIBUTE_PLACEHOLDER \
        in annotation `SdkSuppress`.

        In annotation `SdkSuppress`, parameters `minSdkVersion` or `maxSdkVersion` should not \
        accept the value of 10000. This may be caused by using the version code of an \
        under-development SDK.

        Please replace the invalid version code with versions of finalized SDKs.
    """
        .trimIndent()
        .replace("\\\n", "")
    ...
    // lint detector.
    private val INVALID_VERSION_STRING_LIST = listOf("CUR_DEVELOPMENT")
  }
}
