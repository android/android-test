...
private fun View.generateBitmap(bitmapFuture: ResolvableFuture<Bitmap>) {
  ...
  when {
    ...
    this is SurfaceView -> generateBitmapFromSurfaceViewPixelCopy(destBitmap, bitmapFuture)
      ...
      if (surface != null) {
        generateBitmapFromPixelCopy(surface, destBitmap, bitmapFuture)
      } else {
        bitmapFuture.setException(IllegalStateException("Could not find surface for view."))
      }
    }
  }
}
...
private fun View.getSurface(): Surface {
  ...
  val viewRootImpl = ReflectiveMethod<Object>(View::class.java, "getViewRootImpl").invoke(this)
    ...
    Consumer<PixelCopy.Result> { result ->
      if (result.status == PixelCopy.SUCCESS) {
        ...
        bitmapFuture.setException(
          RuntimeException(String.format("PixelCopy failed: %d", result.status))
        )
      }
    }
