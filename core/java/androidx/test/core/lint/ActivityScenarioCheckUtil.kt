...
fun closedWithTryWithResources(node: UCallExpression): Boolean {
...
fun closedWithRunBlock(node: UCallExpression): Boolean {
...
fun closedManually(node: UCallExpression, closedScenarioReferences: Set<PsiElement>): Boolean {
  val launchedScenario = getActivityScenarioByLaunchCall(node) ?: return false
  ...
  if (referenceOfLaunch == null) return false

  // Checks whether there is a pair of launch() and close() which refers to the same
  // ActivityScenario instance.
  return closedScenarioReferences.contains(referenceOfLaunch)
}

/** Searches ActivityScenario.launch() and ActivityScenario.close(). */
fun getScenarioActivityCalls(
  root: UElement,
  launchCalls: MutableSet<UCallExpression>,
  closeCalls: MutableSet<UCallExpression>
) {
  val launchCallVisitor =
    object : AbstractUastVisitor() {
      override fun visitCallExpression(node: UCallExpression): Boolean {
        if (node.matchMethodSignature(ActivityScenarioConstants.LAUNCH_METHOD_SIGNATURE)) {
          launchCalls.add(node)
        } else if (node.matchMethodSignature(ActivityScenarioConstants.CLOSE_METHOD_SIGNATURE)) {
          closeCalls.add(node)
        }
        // Stops search.
        return true
