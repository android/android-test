...
open class UnclosedActivityScenarioCheck : Detector(), SourceCodeScanner {
  ...
  /** Checks whether the call is under `ActivityScenarioRule` method. */
  private fun inActivityScenarioRule(node: UCallExpression): Boolean {
    val methodNode = node.getParentOfType<UMethod>() ?: return false
    return methodNode.name == "ActivityScenarioRule"
  }
