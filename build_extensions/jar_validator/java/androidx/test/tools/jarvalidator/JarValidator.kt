...
fun main(args : Array<String>) {
    require(args.size >= 3) { "Usage: <output txt> <input jar> <expected prefix 1> <expected prefix 2> ..." }
    ...
    if (nonMatchingEntries.size > 0) {
        nonMatchingEntries.sort()
        val error = "Error: The following classes in ${jarFile.name} did not match one of the expected prefixes $expectedPrefixes \n" + nonMatchingEntries.joinToString("\n")
...
fun matchesExpectedPrefixes(expectedPrefixes: List<String>, className: String): Boolean {
